  ...
  .directive('prospectCard', function() {
    return {
      ...
      template: clientArray.filter(function(item){
              ...
              return '<div class="client-card new-client">' +
              '<label class="select-opp">Select Opportunity: <input type=checkbox></label>' +
  ...
  .directive('clientCard', function() {
    return {
      ...
      template: clientArray.filter(function(item){
              ...
              return '<div class="client-card existing-client">' +
              '<label class="select-opp">Select Opportunity: <input type=checkbox></label>' +
              '<img src="' + value.image_url + '">' +
                '<h3>' + value.status + '</h3>' +
                '<a href="' + value.url + '">Team Website</a></div>'
